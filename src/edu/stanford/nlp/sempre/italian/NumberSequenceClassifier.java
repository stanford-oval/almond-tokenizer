...
public class NumberSequenceClassifier extends AbstractSequenceClassifier<CoreLabel> {
  ...
  public static final Pattern CURRENCY_WORD_PATTERN = Pattern.compile(
      "dollar[oi]|cent|euro|penny|pence|yen|yuan|won",
      Pattern.CASE_INSENSITIVE);
  ...
  private static final Pattern PART_OF_DAY_PATTERN = Pattern.compile("mezzogiorno|mezzodì|mezzanotte", Pattern.CASE_INSENSITIVE);
    ...
    for (int i = 0, sz = pl.size(); i < sz; i++) {
      ...
      if (CURRENCY_SYMBOL_PATTERN.matcher(me.word()).matches() &&
        ...
        if (prev.tag() != null && prev.tag().equals("CD") &&
          ...
          String wd = prev.word();
          if (wd.equalsIgnoreCase("cento") ||
              wd.equalsIgnoreCase("mille") ||
              wd.equalsIgnoreCase("millione") ||
              wd.equalsIgnoreCase("millioni") ||
              wd.equalsIgnoreCase("miliardo") ||
