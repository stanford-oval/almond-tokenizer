...
/**
 * A copy of the same class in edu.stanford.corenlp.pipeline, with some
 * bug fixes.
 * 
 * 
 * An annotator which picks quotations out of the given text. Allows
 * for embedded quotations so long as they are either directed unicode quotes or
 * are
 ...
 * “”,‘’, «», ‹›, 「」, 『』, „”, and ‚’.
 *
 * Note: extracts everything within these pairs as a whole quote segment, which
 * may or may
 * not be the desired behaviour for texts that use different formatting styles
 * than
 ...
 * <ul>
 * <li>singleQuotes: "true" or "false", indicating whether or not to consider '
 * tokens
 * to be quotation marks (default=false).</li>
 * <li>maxLength: maximum character length of quotes to consider
 * (default=-1).</li>
 * <li>asciiQuotes: "true" or "false", indicating whether or not to convert all
 * quotes
 * to ascii quotes before processing (can help when there are errors in quote
 * directionality)
 * (default=false).</li>
 * <li>allowEmbeddedSame: "true" or "false" indicating whether or not to allow
 * smart/directed
 * (everything except " and ') quotes of the same kind to be embedded within one
 * another
 * (default=false).</li>
 * <li>extractUnclosedQuotes: "true" or "false" indicating whether or not to
 * extract unclosed
 * quotes. If "true", an UnclosedQuotationsAnnotation that is structured exactly
 * the same as
 * the QuotationsAnnotation will be added to the document. Any nested unclosed
 * quotations will be
 * contained in nested UnclosedQuotationsAnnotation on the target unclosed
 * quotation
 * (default=false).</li>
 * </ul>
 *
 ...
 * <li>TokensAnnotation (if the tokenizer is run before the quote
 * annotator)</li>
 * <li>TokenBeginAnnotation (if the tokenizer is run before the quote
 * annotator)</li>
 * <li>TokenEndAnnotation (if the tokenizer is run before the quote
 * annotator)</li>
 * <li>SentenceBeginAnnotation (if the sentence splitter has bee run before the
 * quote annotator)</li>
 * <li>SentenceEndAnnotation (if the sentence splitter has bee run before the
 * quote annotator)</li>
 * </ul>
 *
 *
...
public class QuoteAnnotator implements Annotator {
  ...
  /**
   * Return a QuoteAnnotator that isolates quotes denoted by the
   * ASCII characters " and ' as well as a variety of smart and international
   * quotes.
   * If an unclosed quote appears, by default, this quote will not be counted as
   * a quote.
   *
   * @param props
   *          Properties object that contains the customizable properties
  ...
  public void annotate(Annotation annotation) {
    ...
    if (SMART_QUOTES) {
      ...
      List<CoreMap> cmQuotesUnicode = getCoreMapQuotes(overall.first(), tokens, sentences, text, docID, false);
      ...
      overall = getQuotes(quotesFrom);
      docID = annotation.get(CoreAnnotations.DocIDAnnotation.class);
      List<CoreMap> cmQuotesAsciiNoSingle = getCoreMapQuotes(overall.first(), tokens, sentences, text, docID, false);
  ...
  private static final Pattern asciiSingleQuote = Pattern
      .compile("&apos;|[\u0091\u2018\u0092\u2019\u201A\u201B\u2039\u203A']");
  private static final Pattern asciiDoubleQuote = Pattern
      .compile("&quot;|[\u0093\u201C\u0094\u201D\u201E\u00AB\u00BB\"]");
  ...
  public Pair<List<Pair<Integer, Integer>>, List<Pair<Integer, Integer>>> recursiveQuotes(String text, int offset,
      String prevQuote) {
    ...
    for (String qKind : quotesMap.keySet()) {
      for (Pair<Integer, Integer> q : quotesMap.get(qKind)) {
        if (q.second() - q.first() >= qKind.length() * 2) {
          ...
          Pair<List<Pair<Integer, Integer>>, List<Pair<Integer, Integer>>> embedded = recursiveQuotes(toPass,
